<?php
// login.php - AUTHENTICATION MODULE
session_start();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($username) || empty($password)) {
        $errorMsg = "ОШИБКА: ВВЕДИТЕ ДАННЫЕ";
    } else {
        $stmt = $pdo->prepare("SELECT id, username, password_hash, access_level FROM users WHERE username = ?");
        ...
        if ($user && password_verify($password, $user['password_hash'])) {
            ...
            $errorMsg = "ОТКАЗ В ДОСТУПЕ: НЕВЕРНЫЕ ДАННЫЕ";
        }
    }
}
?>
...
<head>
    ...
    <title>АВТОРИЗАЦИЯ COL</title>
    <link href="https://fonts.googleapis.com/css2?family=Share+Tech+Mono&display=swap" rel="stylesheet">
    <style>
        /* Те же стили для единообразия */
        ...
        .login-box::before { content: "AUTH_REQUIRED"; position: absolute; top: -10px; left: 20px; background: var(--bg); padding: 0 10px; color: var(--alert); }
        ...
        .alert { color: var(--alert); text-align: center; margin-bottom: 15px; border: 1px dashed var(--alert); padding: 10px; animation: blink 1s infinite; }
...
<body>
    ...
    <div class="login-box">
        ...
        <form method="POST">
            ...
            <button type="submit" class="btn">ИДЕНТИФИКАЦИЯ</button>
        </form>
        
        <a href="register.php" class="footer-link">[ НЕТ ДОСЬЕ? РЕГИСТРАЦИЯ ]</a>
        <a href="index.php" class="footer-link">[ ОТМЕНА ]</a>
    </div>
</body>
</html>