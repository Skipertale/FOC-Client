...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_rule'])) {
    $rule = $_POST['rule'];
    $hub_id = $_POST['hub_id'] ?? 4; // Указываем hub_id = 2 для хаба 2
...
try {
    $rules = $pdo->prepare("SELECT * FROM rules WHERE hub_id = 4"); // Фильтрация правил для хаба 1
    $rules->execute();
    $rules = $rules->fetchAll(PDO::FETCH_ASSOC);
    $applications = $pdo->prepare("SELECT * FROM applications WHERE hub_id = 4"); // Заявки для хаба 1
...
// Получение данных из базы данных
try {
    $rules = $pdo->prepare("SELECT * FROM rules WHERE hub_id = 4"); // Фильтрация правил для хаба 2
    $rules->execute();
    $rules = $rules->fetchAll(PDO::FETCH_ASSOC);
} catch (PDOException $e) {
    die("Ошибка при получении данных: " . $e->getMessage());
...
<body>
    <header>
        ...
        <nav>
            <a href="logouth4.php" class="logout">Выйти</a>
        </nav>
    </header>
