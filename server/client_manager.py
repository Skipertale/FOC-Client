...
class ClientManager:
    ...
    class Client:
        ...
        def __init__(self, server, transport, user_id, ipid):
            ...
            # SFX preset (set via /sfx, played on next IC message)
            self.sfx_url = ""
            self.sfx_looping = "0"
            ...
            self.playing_audio = {}  # channel → name

            # Personal toggle for voiced IC playback. When enabled, incoming
            # IC packets may replace their SFX track with generated speech.
        ...
        def change_character(self, char_id, force=False):
            ...
            achievements.check_char_switch(self.server, self)
                ...
                for area in target_areas:
                    ...
                    if self.edit_ambience:
                        if self.is_mod or self in area.owners:
                            slot = getattr(self, "edit_ambience_slot", 1)
                            area.set_ambience(name, slot=slot)
                            self.send_ooc(
                                f"Setting area [{area.id}] {area.name} ambience slot {slot} to {name}."
                            )
                            continue
                        else:
        ...
        def set_area(self, area, target_pos=""):
            ...
            broadcast_event("move", {
                ...
                "who": getattr(self, "name", "?"),
                "char": getattr(self, "char_name", ""),
                "what": f"{getattr(old_area, 'name', '?')} → {area.name}",
            })
        ...
        def change_area(self, area, password=""):
            ...
            achievements.check_area_change(self.server, self, self.area.id)
        ...
        def disemvowel_message(self, message):
            ...
            message = re.sub("[aeiouуеыаоэяёию]", "", message, flags=re.IGNORECASE)
