...
class Area:
    ...
    #TODO add OOC and IC slow mode timer
    ...
    def load(self, area):
        ...
        if "ambience" in area:
            ...
            if isinstance(raw, str):
                self.ambience = {1: raw} if raw else {}
            elif isinstance(raw, dict):
                self.ambience = {int(k) if not isinstance(k, int) else k: v for k, v in raw.items() if v}
            else:
                self.ambience = {}
        if "can_dj" in area:
        ...
        for slot, name in self.ambience.items():
            if name:
                self._send_ambience(name, slot)
        if self.music_autoplay:
            for client in self.clients:
                if self.music != client.playing_audio.get(0):
                    client.send_command(
    ...
    def new_client(self, client):
        ...
        if self.music_autoplay and self.music != client.playing_audio.get(0):
            client.send_command(
                "MC", self.music, -1, "", self.music_looping, 0, self.music_effects
            )
        ...
        for slot, name in self.ambience.items():
            if name and client.playing_audio.get(slot) != name:
                client.send_command(
                    "MC",
                    name,
                    -1,
    ...
    def remove_client(self, client):
        ...
        # Fade out main music (channel 0)
        if self.music and client.playing_audio.get(0):
            client.send_command("MC", "~stop.mp3", -1, "", self.music_looping, 0, int(MusicEffect.FADE_OUT))
        # Stop all active ambience channels for this client
        for slot, name in self.ambience.items():
            if name:
                client.send_command("MC", "~stop.mp3", -1, "", 1, slot, int(MusicEffect.FADE_OUT))
    ...
    def send_ic(self,
        ...
        # Player typed SFX directly in IC field - serve via MC from server
        if client and not client.sfx_url and sfx and not sfx.startswith(("http://", "https://")):
            sfx_name = os.path.basename(sfx.rsplit("/", 1)[-1].rsplit("\\", 1)[-1])
            sfx_dir = "storage/sfx"
            if os.path.isdir(sfx_dir):
                for f in os.listdir(sfx_dir):
                    if os.path.splitext(f)[0].lower() == os.path.splitext(sfx_name)[0].lower():
                        local_path = "sfx/" + os.path.basename(f)
                        sfx_http = getattr(self.server, "_sfx_http_url", "")
                        if sfx_http:
                            client.sfx_url = sfx_http.rstrip("/") + "/" + local_path
                        elif self.server.config.get("asset_url", ""):
                            client.sfx_url = self.server.config["asset_url"].rstrip("/") + "/" + local_path
                        if client.sfx_url:
                            sfx = ""
                        break

        # Inject saved SFX filename into MS packet (only for non-URL local files)
        if client and getattr(client, "sfx_url", ""):
            sfx_val = client.sfx_url
            if not sfx_val.startswith(("http://", "https://")):
                sfx = sfx_val
                sfx_looping = "0"
                if client.sfx_looping in ("0", 0, ""):
                    client.sfx_url = ""
        ...
        tts_sfx = None
        tts_sfx_delay = 0
        if client is not None and any(getattr(c, "tts_enabled", False) for c in targets):
            speaker_name = showname
            if speaker_name == "":
                if cid != -1:
                    try:
                        speaker_name = self.area_manager.char_list[cid]
                    except Exception:
                        speaker_name = ""
                if speaker_name == "":
                    speaker_name = (
                        getattr(client, "showname", "")
                        or getattr(client, "char_name", "")
                        or getattr(client, "name", "Кто-то")
                    )
            tts_text = self.server.build_tts_line(speaker_name, msg)
            if tts_text:
                tts_sfx = self.server.generate_tts_asset(tts_text)
        ...
        for c in targets:
            ...
            target_sfx = sfx
            target_sfx_delay = sfx_delay
            target_sfx_looping = sfx_looping
            if getattr(c, "tts_enabled", False) and tts_sfx:
                target_sfx = tts_sfx
                target_sfx_delay = tts_sfx_delay
                target_sfx_looping = "0"
            # Inject /sfx URL into MS sfx field (client plays via sfx_player, not MC)
            if client and getattr(client, "sfx_url", ""):
                target_sfx = client.sfx_url
                target_sfx_looping = getattr(client, "sfx_looping", "0")
                if target_sfx_looping in ("0", 0, ""):
                    client.sfx_url = ""
                    client.sfx_looping = "0"
    ...
    def set_ambience(self, name, slot=1):
        if name:
            self.ambience[slot] = name
        else:
            self.ambience.pop(slot, None)
        self._send_ambience(name, slot)

    def _send_ambience(self, name, slot=1):
        self.send_command(
            "MC",
            name or "",
            -1,
    ...
    def change_background(self, bg, overlay="", mode=1):
        ...
        if self.use_backgrounds_yaml:
            ...
            valid_bgs = [b.lower() for b in self.server.backgrounds]
            if bg.lower() not in valid_bgs:
                # Check if it exists in storage/backgrounds/ as a folder
                bg_path = os.path.join("storage", "backgrounds", bg)
                if os.path.isdir(bg_path):
                    self.server.backgrounds.append(bg)
                    if "Server" in self.server.backgrounds_categories:
                        self.server.backgrounds_categories["Server"].append(bg)
                    else:
                        self.server.backgrounds_categories["Server"] = [bg]
                else:
                    raise AreaError(
