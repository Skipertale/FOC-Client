...
class PG1(commands.Bot):
    ...
    def queue_message(self, name, message, charname, anim):
        ...
        anim_url = None
        embed_emotes = False
        if "base_url" in self.server.config["pg1"]:
            base = self.server.config["pg1"]["base_url"]
        if "embed_emotes" in self.server.config["pg1"]:
            embed_emotes = self.server.config["pg1"]["embed_emotes"]
        if base is not None:
            avatar_url = base + \
    ...
    async def on_ready(self):
        ...
        while True:
            ...
            await asyncio.sleep(max(0.1, self.server.config["pg1"]["tickspeed"]))
