...
class CR2(commands.Bot):
    ...
    def queue_message(self, name, message, charname, anim):
        ...
        anim_url = None
        embed_emotes = False
        if "base_url" in self.server.config["cr2"]:
            base = self.server.config["cr2"]["base_url"]
        if "embed_emotes" in self.server.config["cr2"]:
            embed_emotes = self.server.config["cr2"]["embed_emotes"]
        if base is not None:
            avatar_url = base + \
    ...
    async def on_ready(self):
        ...
        while True:
            ...
            await asyncio.sleep(max(0.1, self.server.config["cr2"]["tickspeed"]))
