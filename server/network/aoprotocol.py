...
import traceback
import logging
from server import lockdown_check
from server.word_filter import apply as apply_word_filter
from server.word_filter import apply_check as apply_word_filter_check
from server import automod
...
class AOProtocol(asyncio.Protocol):
    ...
    def connection_lost(self, exc):
        ...
        if self.client is not None:
            achievements.check_disconnect(self.server, self.client)
    ...
    def net_cmd_hi(self, args):
        ...
        ipid = self.client.ipid
        ip_address = getattr(self.client, "ip_address", "")
        ...
        if wl_cfg.get("enabled", False) and not wl_mod.is_allowed(hdid, ipid, ip_address):
            msg = "Вы не находитесь в вайт-листе сервера.\r\n"
            invite = wl_mod.get_invite()
            if invite:
                msg += f"Присоединитесь к нашему Discord: {invite}\r\n"
            msg += "Для получения доступа обратитесь к администрации."
            self.client.send_command("BD", msg)
            self.client.disconnect()
            wl_bot = getattr(self.server, "discord_bot", None)
            if wl_bot is not None and getattr(wl_bot, "whitelist_enabled", False):
                asyncio.ensure_future(
                    wl_bot.notify_join_attempt(self.client, hdid, ipid, ip_address),
                    loop=self.server.loop,
                )
            return

        self.client.is_checked = True
        achievements.check_login(self.server, self.client)
        database.log_connect(self.client, failed=False)
        broadcast_event("connect", {
            "time": time.strftime("%H:%M:%S"),
            "who": self.client.name,
            "char": self.client.char_name,
            "showname": self.client.showname,
            "what": f"Подключился из IPID {self.client.ipid}",
        })
        self.client.send_command(
    ...
    def net_cmd_ms(self, args):
        ...
        # Apply DB-based word filters (webpanel UI) + automod
        if self.client.area.area_manager.censor_ic:
            hub_name = self.client.area.area_manager.name
            area_name = self.client.area.name
            text, wf_should_warn = apply_word_filter_check(text, hub_name, area_name)
            if len(showname) > 0:
                showname = apply_word_filter(showname, hub_name, area_name)
            if wf_should_warn:
                automod.warn(self.client, "Нецензурная лексика")
            automod.check_all(self.client, text, hub_name, area_name)
        if text.lower().startswith("/a ") or text.lower().startswith("/s "):
            ...
            try:
                ...
                return
        # /p — queue preset command in IC (только добавление)
        if text.lstrip().startswith("/p") and not text.lstrip().startswith("/p_"):
            is_authorized = self.client.is_mod or self.client in self.client.area.owners or self.client in self.client.area.area_manager.owners
            if not is_authorized:
                self.client.send_ooc("[Queue] /p — только для модеров, CM и GM.")
                return
            qm = self.client.server.queue_manager
            if not qm:
                self.client.send_ooc("[Queue] Очередь не инициализирована.")
                return
            arg = text.lstrip()
            if arg == "/p":
                self.client.send_ooc("[Queue] Укажи текст сообщения. /p [сек] текст")
                return
            arg = arg[3:].strip()
            delay = 0
            msg = arg
            ...
            if not msg:
                self.client.send_ooc("[Queue] Укажи текст сообщения. /p [сек] текст")
                return
            # Сохраняем текущие folder, sfx и pos из MS-пакета
            qm.add(self.client, msg, delay,
                   folder=folder, sfx=sfx, sfx_delay=sfx_delay, anim=anim,
                   pos=pos, pre=pre, emote_mod=emote_mod,
                   button=button, evidence=evidence, flip=flip,
                   ding=ding, color=color)
            if delay > 0:
                self.client.send_ooc(f"[Queue] Добавлено (отпр. через {delay}c): {msg[:50]}")
            else:
                self.client.send_ooc(f"[Queue] Добавлено в очередь: {msg[:50]}")
            return
        ...
        if whisper_clients is None:
            ...
            if (
                ...
                # Discord Bridgebot — route to all enabled bridge sections
                bridge_sections = getattr(self.server.discord_bot, "bridge_sections", {})
                for section, cfg in bridge_sections.items():
                    hub_id = cfg.get("hub_id", 0)
                    area_id = cfg.get("area_id", 0)
                    if self.client.area.area_manager.id == hub_id and self.client.area.id == area_id:
                        ...
                        if not text.strip():
                            text = "_ _"
                        self.server.discord_bot.queue_message(
                            webname, text, self.client.char_name, anim, section
                        )
                        break
        ...
        if "¨" in text:
            ...
            achievements.check_ic_message(self.server, self.client)
            return
    ...
    def net_cmd_ct(self, args):
        ...
        if self.client.area.area_manager.censor_ooc:
            hub_name = self.client.area.area_manager.name
            area_name = self.client.area.name
            args[0] = apply_word_filter(args[0], hub_name, area_name)
            args[1], wf_should_warn = apply_word_filter_check(args[1], hub_name, area_name)
            if wf_should_warn:
                automod.warn(self.client, "Нецензурная лексика")
            automod.check_all(self.client, args[1], hub_name, area_name)
        ...
        achievements.check_ooc_message(self.server, self.client)
    ...
    def net_cmd_zz(self, args):
        ...
        if self.client.char_id is None:
            self.client.send_command("KK", "Пашол нахуй чурка")
            self.client.disconnect()
            return
        ...
        if len(args) < 1:
            ...
            achievements.check_modcall(self.server, self.client)
        else:
            self.server.send_all_cmd_pred(
                "ZZ",
