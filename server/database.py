...
class Database:
    ...
    def migrate_json_to_v1(self):
        ...
        with self.db as conn:
            ...
            with open("migrations/v1.sql", "r", encoding="utf-8") as file:
    ...
    def migrate_to_version(self, version):
        with self.db as conn:
            ...
            with open(f"migrations/v{version}.sql", "r", encoding="utf-8") as file:
                conn.executescript(file.read())
        logger.debug("Migration to v%s complete", version)

    def ipaddr(self, ipid):
        """Get an IP address from an IPID."""
        with self.db as conn:
            ip = conn.execute(
                dedent(
                    """
    ...
    def set_player_code(self, hdid, code, created_by=""):
        """Assign a player access code to an HDID."""
        with self.db as conn:
            conn.execute(
                dedent(
                    """
                INSERT OR REPLACE INTO player_access_codes(code, hdid, created_by)
                VALUES (?, ?, ?)
                """
                ),
                (code, hdid, created_by),
            )

    def get_player_code(self, hdid):
        """Get the access code for an HDID."""
        with self.db as conn:
            row = conn.execute(
                dedent(
                    """
                SELECT code FROM player_access_codes WHERE hdid = ?
                """
                ),
                (hdid,),
            ).fetchone()
    ...
    def get_hdid_by_player_code(self, code):
        """Get HDID by access code."""
        with self.db as conn:
            row = conn.execute(
                dedent(
                    """
                SELECT hdid FROM player_access_codes WHERE code = ?
                """
                ),
                (code,),
            ).fetchone()
            return row["hdid"] if row else None

    def remove_player_code(self, hdid):
        """Remove the access code for an HDID."""
        with self.db as conn:
            conn.execute(
                dedent(
                    """
                DELETE FROM player_access_codes WHERE hdid = ?
                """
                ),
                (hdid,),
            )
