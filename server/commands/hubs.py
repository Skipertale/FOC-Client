...
__all__ = [
    ...
    "ooc_cmd_rename_hub",
    "ooc_cmd_ban_in_hub",
    "ooc_cmd_unban_in_hub",
    "ooc_cmd_add_owner",
    "ooc_cmd_remove_owner",
...
def ooc_cmd_hub(client, arg):
    ...
    banned_clients = load_banned_clients()
    ...
    try:
        for hub in client.server.hub_manager.hubs:
            ...
            if (
                ...
                hub_id = str(hub.id)
                if hub_id in banned_clients and any(
                    entry['hdid'] == client.hdid or entry['ipid'] == client.ipid
                    for entry in banned_clients[hub_id]
                ):
                    reason = next(
                        (entry['reason'] for entry in banned_clients[hub_id] if entry['hdid'] == client.hdid or entry['ipid'] == client.ipid),
                        "Причина неизвестна"
                    )
                    client.send_ooc(f"Вы не можете войти в [{hub.id}] {hub.name}, так как вы забанены. Для разбана обратитесь к владельцу хаба.")
                    client.send_ooc(f"Причина: {reason}.")
                    return
...
def load_banned_clients():
    """Загрузка забаненных клиентов из файла."""
    if os.path.exists("banned_clients.yaml"):
        with open("banned_clients.yaml", "r") as file:
            print("[DEBUG] Файл banned_clients.yaml успешно загружен.")
            return yaml.safe_load(file) or {}
    print("[DEBUG] Файл banned_clients.yaml отсутствует.")
    return {}

def save_banned_clients(banned_clients):
    """Сохранение забаненных клиентов в файл."""
    with open("banned_clients.yaml", "w") as file:
        yaml.safe_dump(banned_clients, file)

def load_hub_owners():
    """Загрузка владельцев хабов из файла."""
    if os.path.exists("hub_owners.yaml"):
        with open("hub_owners.yaml", "r") as file:
            print("[DEBUG] Файл hub_owners.yaml успешно загружен.")
            return yaml.safe_load(file) or {}
    print("[DEBUG] Файл hub_owners.yaml отсутствует.")
    return {}

def save_hub_owners(hub_owners):
    """Сохранение владельцев хабов в файл."""
    with open("hub_owners.yaml", "w") as file:
        yaml.safe_dump(hub_owners, file)

def ooc_cmd_ban_in_hub(client, args):
    """
    Команда для бана игрока в текущем хабе.
    Формат команды:
    /ban_in_hub <client.id> <reason>
    """
    hub_id = str(client.area.area_manager.id)

    banned_clients = load_banned_clients()
    hub_owners = load_hub_owners()

    # Проверяем, является ли клиент владельцем текущего хаба
    if not any(owner['hdid'] == client.hdid for owner in hub_owners.get(hub_id, [])):
        client.send_ooc("$H: У вас нет прав на использование этой команды.")
        return

    if not args:
        client.send_ooc("$H: Укажите ID клиента и причину для бана.")
        return
    ...
    target_id, reason = args_split

    try:
        target_id = int(target_id)
    except ValueError:
        client.send_ooc("$H: Некорректный формат ID клиента. Укажите числовой ID.")
        return

    target_client = next((c for c in client.area.clients if c.id == target_id), None)
    if not target_client:
        client.send_ooc("$H: Игрок с указанным ID не найден в текущей области.")
        return

    if hub_id not in banned_clients:
        banned_clients[hub_id] = []

    for ban_entry in banned_clients[hub_id]:
        if ban_entry['hdid'] == target_client.hdid:
            client.send_ooc("$H: Этот игрок уже забанен в этом хабе.")
            return

    ban_id = str(uuid.uuid4())
    ban_entry = {'id': ban_id, 'hdid': target_client.hdid, 'ipid': target_client.ipid, 'reason': reason}
    banned_clients[hub_id].append(ban_entry)
    save_banned_clients(banned_clients)

    client.send_ooc(f"Игрок {target_client.name} забанен в этом хабе. Причина: {reason}.")
    client.send_ooc(f"ID бана: {ban_id}")
    target_client.send_ooc(f"Вы были забанены в этом хабе. Причина: {reason}. Для разбана обратитесь к владельцу хаба.")
    target_client.send_ooc(f"ID бана: {ban_id}")

    hub_manager = client.server.hub_manager
    default_hub = hub_manager.get_hub_by_id(0)
    if default_hub:
        target_client.change_area(default_hub.default_area())
    else:
        client.send_ooc("$H: Ошибка: Хаб с ID 0 не найден.")

def ooc_cmd_add_owner(client, args):
    """
    Команда для добавления владельца хаба.
    Формат команды:
    /add_owner <client.id>
    """
    ...
    if hub_id not in hub_owners:
        hub_owners[hub_id] = []

    if not args:
        client.send_ooc("$H: Укажите ID клиента для добавления владельца.")
        return
    ...
    for owner in hub_owners[hub_id]:
        if owner['hdid'] == target_client.hdid:
            client.send_ooc("$H: Этот игрок уже является владельцем этого хаба.")
            return

    hub_owners[hub_id].append({'hdid': target_client.hdid, 'ipid': target_client.ipid})
    save_hub_owners(hub_owners)

    client.send_ooc(f"$H: Игрок {target_client.name} добавлен в владельцы хаба {hub_id}.")

def ooc_cmd_remove_owner(client, args):
    """
    Команда для удаления владельца хаба.
    Формат команды:
    /remove_owner <client.id>
    """
    ...
    if hub_id not in hub_owners:
        client.send_ooc("$H: В этом хабе нет владельцев.")
        return

    if not args:
        client.send_ooc("$H: Укажите ID клиента для удаления владельца.")
        return
    ...
    for owner in hub_owners[hub_id]:
        if owner['hdid'] == target_client.hdid:
            hub_owners[hub_id].remove(owner)
            save_hub_owners(hub_owners)
            client.send_ooc(f"$H: Игрок {target_client.name} удалён из владельцев хаба {hub_id}.")
            return

    client.send_ooc("$H: Этот игрок не является владельцем этого хаба.")

def ooc_cmd_unban_in_hub(client, args):
    """
    Команда для разбана игрока в текущем хабе.
    Формат команды:
    /unban_in_hub <ban.id>
    """
    hub_id = str(client.area.area_manager.id)
    ...
    if not args:
        client.send_ooc("$H: Укажите ID бана для разблокировки.")
        return

    ban_id = args.strip()

    if hub_id not in banned_clients:
        client.send_ooc("$H: В этом хабе нет забаненных игроков.")
        return

    for ban_entry in banned_clients[hub_id]:
        if ban_entry['id'] == ban_id:
            banned_clients[hub_id].remove(ban_entry)
            save_banned_clients(banned_clients)
            client.send_ooc(f"$H: Игрок с ID бана {ban_id} был разбанен в хабе {hub_id}.")
            return

    client.send_ooc(f"$H: Бана с ID {ban_id} не найдено в хабе {hub_id}.")
...
# Функция, которая отправляет запрос на подтверждение в Discord
def request_approval(client, arg, cmd):
    client_ipid_str = str(client.ipid)
    client_hdid_str = str(client.hdid)
    print(f"ГМ-запрос: Client IPID={client_ipid_str}, HDID={client_hdid_str}")

    auto_reason = None
    if client.is_mod:
        auto_reason = "игрок является администратором"
    elif client in client.area.area_manager.owners:
        auto_reason = "игрок уже является GM в данном хабе"
    elif _is_whitelisted(client):
        auto_reason = "игрок находится в вайт-листе"

    if auto_reason:
        ooc_cmd_gm_execute(client, arg)
        client.send_ooc(f"[BOT] Автоматическое согласование. {auto_reason.capitalize()}!")
        _notify_auto_approval(client, arg, cmd, auto_reason)
        return

    # Отправляем запрос в Discord через UnifiedBot
    bot = getattr(client.server, "discord_bot", None)
    if bot is None or not getattr(bot, "login_enabled", False):
    ...
    try:
        ...
        asyncio.ensure_future(bot.submit_gm_request(client, arg, cmd), loop=loop)
    except Exception as exc:
        client.send_ooc(f"Не удалось отправить запрос: {exc}")


def _notify_auto_approval(client, arg, cmd, reason):
    notification = (
        f"**Авто-согласование: {cmd}**\n"
        f"Игрок: {client.showname} [{client.id}]\n"
        f"Зона: {client.area.name}, Hub [{client.area.area_manager.id}]\n"
        f"Причина: {reason}\n"
        f"```\n"
        f"IPID: {client.ipid}\n"
        f"HDID: {client.hdid}\n"
    ...
    if bot is None:
        return
    loop = getattr(client.server, "loop", None) or asyncio.get_event_loop()
    asyncio.ensure_future(_send_auto_notify(bot, notification), loop=loop)


def _is_whitelisted(client):
    from server import whitelist as wl_mod
    hdid = str(getattr(client, "hdid", ""))
    ipid = str(getattr(client, "ipid", ""))
    ip = str(getattr(client, "ip_address", ""))
    return wl_mod.gm_is_allowed(hdid, ipid, ip)


async def _send_auto_notify(bot, notification):
    try:
        channel = await bot.get_approval_channel()
        if channel:
            await channel.send(notification)
    except Exception as exc:
        print(f"Ошибка отправки уведомления об авто-согласовании: {exc}")

def ooc_cmd_gm_execute(client, arg):
    ...
    if len(client.area.area_manager.owners) == 0 or client.is_mod or client in client.area.area_manager.owners:
        ...
        client.send_ooc("В этом хабе уже есть ГМ.")

# Новая функция ooc_cmd_gm с добавленным запросом на подтверждение
def ooc_cmd_gm(client, arg):
    if not client.is_mod and not client.area.area_manager.can_gm:
        raise ClientError("You can't become a GM in this Hub!")
    request_approval(client, arg, "/gm")
