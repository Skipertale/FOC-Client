...
def ooc_cmd_edit_ambience(client, arg):
    ...
    tog can be `on`, `off`, `clear`, slot number or empty.
    Usage: /edit_ambience [tog|slot|clear]
    """
    parts = arg.split()
    if len(parts) > 1:
        raise ArgumentError(
            "This command can only take one argument!"
        )
    if parts:
        a = parts[0]
        if a == "on":
        ...
        elif a in ("clear", "stop", "reset"):
            area = client.area
            count = len(area.ambience)
            for slot in list(area.ambience.keys()):
                area.ambience.pop(slot)
                area.send_command("MC", "~stop.mp3", -1, "", 1, slot, int(MusicEffect.FADE_OUT))
            client.send_ooc(f"All ambience cleared ({count} slot(s)).")
            return
        elif a.isdigit():
            slot = int(a)
            if slot < 1 or slot > 5:
                raise ArgumentError("Slot must be between 1 and 10.")
            client.edit_ambience = True
            client.edit_ambience_slot = slot
            client.send_ooc(f"Now editing ambience slot {a}. Playing a song will set this slot.")
            return
        else:
            raise ArgumentError("Invalid argument: {}".format(a))
    else:
        client.edit_ambience = not client.edit_ambience
    stat = "no longer"
    if client.edit_ambience:
        stat = "now"
    client.send_ooc(f"Playing a song will {stat} edit the area's ambience (slot {client.edit_ambience_slot}).")


def ooc_cmd_ea(client, arg):
    """Alias for /edit_ambience. /ea N sets ambience slot N, /ea turns off."""
    ooc_cmd_edit_ambience(client, arg)
...
def ooc_cmd_bg_list(client, arg):
    """
    Show server-side backgrounds available in storage/backgrounds/.
    Usage: /bg_list
    """
    bg_dir = "storage/backgrounds"
    if not os.path.isdir(bg_dir):
        client.send_ooc("No server-side backgrounds found.")
        return
    names = sorted(d for d in os.listdir(bg_dir) if os.path.isdir(os.path.join(bg_dir, d)))
    if not names:
        client.send_ooc("No server-side backgrounds found.")
        return
    msg = "Server backgrounds:\n" + "\n".join(f"  {n}" for n in names)
    client.send_ooc(msg)


@mod_only(area_owners=True)
def ooc_cmd_bg_save(client, arg):
    """
    Download a background image from a URL and save it to the server.
    Usage: /bg_save <name> <position> <url>
    The position is used as the PNG filename (e.g., 'default' → default.png).
    A design.ini is auto-created with the position listed.
    """
    parts = arg.strip().split(None, 2)
    if len(parts) < 3:
        raise ArgumentError("Usage: /bg_save <name> <position> <url>")
    ...
    bg_dir = os.path.join("storage", "backgrounds", name)
    os.makedirs(bg_dir, exist_ok=True)

    img_path = os.path.join(bg_dir, pos + ".png")
    try:
    ...
    # Update design.ini
    ini_path = os.path.join(bg_dir, "design.ini")
    positions = []
        ...
        with open(ini_path, "r", encoding="utf-8") as f:
            for line in f:
                ...
                if line.startswith("positions"):
                    ...
                    positions = [p.strip() for p in existing.split(",") if p.strip()]
    if pos not in positions:
        positions.append(pos)
    with open(ini_path, "w", encoding="utf-8") as f:
        f.write("scaling = smooth\n")
        f.write("positions = " + ", ".join(positions) + "\n")

    # Add to server's background list if not already there
    if name.lower() not in (b.lower() for b in client.server.backgrounds):
        client.server.backgrounds.append(name)
        if "Server" in client.server.backgrounds_categories:
            client.server.backgrounds_categories["Server"].append(name)
        else:
            client.server.backgrounds_categories["Server"] = [name]

    database.log_misc("bg.save", client, data={"name": name, "pos": pos, "url": url})
    client.send_ooc(f"Фон '{name}' сохранён (позиция: {pos}).")
